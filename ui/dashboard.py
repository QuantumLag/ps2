...
# Add parent directory to path so we can import core, models, data, utils
...
def load_system():
    ...
    try:
        ...
        agents_list = loader.load_agents(str(data_dir / "agents.csv"))
        orders_list = loader.load_orders(str(data_dir / "orders.csv"))
        environment = loader.load_environment(str(data_dir / "environment_edges.csv"))

        # Initialize state manager with loaded agents
        state_manager = StateManager(agents=agents_list)
        
        # Add orders to queue
        state_manager.add_orders_to_queue(orders_list)
...
def render_environment_map(agents: List[Agent], orders: List[Order], assignments: dict):
    ...
    order_colors = [
        "green" if o.order_id in assignments else "red"
        for o in orders
    ]
    ...
    try:
        with st.spinner("🔄 Running optimization..."):
            ...
            for order_id, agent_id in result.assignments.items():
                # Find agent and add order to active_orders
                agent = next((a for a in agents if a.agent_id == agent_id), None)
                if agent:
